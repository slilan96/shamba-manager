...
describe('\'users\' service', () => {
  ...
  describe('POST creating a user', () => {
    ...
    it('should reject user creation if one of the required fields is missing', async () => {
      ...
      // when
      const response = app.service('users').create(_.omit(validUserInfo, ommittedField), { authenticated: true });
    ...
    it('should not return password field to the user after creation for an external request', async () => {
      // when
      const response = await app.service('users').create(validUserInfo, { provider: 'external', authenticated: true });
    ...
    it('should reject user creation from an unauthenticated external request', async () => {
      // when
      const response = app.service('users').create(validUserInfo, { disableRateLimit: true, provider: 'external' });
