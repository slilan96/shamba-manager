...
describe("Migrations Tests", () => {
  ...
  describe("Creating the 'farms' table", () => {
    it("should create the farms table", () =>
      expect(
        knexInstance,
        "to apply migration",
        "20200524231606_init_farms.js",
      ).then(() =>
        expect(knexInstance, "to have table", "farms").and("to have columns", {
          farms: [
            "id",
            "farm_name",
            "title_number",
  ...
  describe("Adding 'national_id' column to staff table", () => {
    it("should add the national_id field to the staff table", () =>
      expect(
        ...
        expect(knexInstance, "to have column", { staff: "national_id" }),
      ));
  });
  ...
  describe("Dropping 'national_id' column from staff table", () => {
    it("should drop the national_id column from the staff table", () =>
      expect(
        ...
        expect(knexInstance, "not to have column", { staff: "national_id" }),
      ));
  });

  describe("Adding 'date_of_harvest' column to harvests table", () => {
    it("should add a date_of_harvest_column to the harvests table", () =>
      expect(
        knexInstance,
        "to apply migration",
        "20200806143339_add_date_of_harvest_column_to_harvest_table.js",
      ).then(() =>
        expect(knexInstance, "to have column", { harvests: "date_of_harvest" }),
      ));
  });
  ...
  describe("Adding 'project_id' field to harvests table", () => {
    it("should add the project_id field to the harvests table", () =>
      expect(
        ...
        expect(knexInstance, "to have column", { harvests: "project_id" }),
      ));
  });
  ...
  describe("Dropping project_id column from harvests table", () => {
    it("should drop the project_id column from the harvests table", () =>
      expect(
        ...
        expect(knexInstance, "not to have column", { harvests: "project_id" }),
      ));
  });
});
